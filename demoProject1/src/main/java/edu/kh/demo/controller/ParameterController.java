...
public class ParameterController {
	...
	public String paramTest2(@RequestParam("title") String title,
							...
							@RequestParam(value="publisher", required = false, defaultValue = "kh정보교육원") String publisher
											// required = false (꼭 파라미터를 제출하지 않아도 된다) defaultValue = "" (단, 제출하지 않았다면 기본값을 이걸로 지정할것.) 
											// required = true인 파라미터를 빼먹고 제출하면 400(Bad Request) 발생
											// ※ ""(빈문자열) 일경우에는 에러 발생 X
	...
	// 4. @ModelAttribute를 이용한 파리미터 얻어오기
	// @ModelAttribute
	// -DTO( 또는 VO)와 같이 사용하는 어노테이션
	//전달받은 파라미터의 name속성값이
	// 함꼐 사용되는 DTO의 필드명과 같다면
	...
	public String paramTest4(@ModelAttribute MemberDTO inputMember) {
		...
		//(현재위치가 중요! -> 현재 경로의 가장 마지막 부분을 redirect: 이주소 로 변경하기 때문에)
