...
// - 자원(데이터, 파일) 을 이름(주소)으로 구분하여 자원의 상태(State)를 주고받은것
...
const addComment = document.quearySelector("#addComment"); //button

addComment.addEventListener("click", e=>{

    // 로그인이 되어있지 않은 경우
    if(loginMemberNo == null){ // boardDetail.html 내부 th:inline 내부 전역변수
        alert("로그인 후 이용해주세요");
        return;
    }
...
const showInsertComment = (parentCommentNo, btn) => {
    ...
    // 매개변수에 +문자열+ 작성시 Number타입으로 형변환하라
...
const updateComment = (commentNo, btn) => {
    ...
    const textarea = btn.parentElement.previousElementSibling; // 부모요소 타고 올라가서 sibling 선택
