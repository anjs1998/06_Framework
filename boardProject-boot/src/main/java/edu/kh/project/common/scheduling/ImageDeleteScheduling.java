...
@Slf4j
@Component // 별다른 역할없이 Bean으로만 등록하는 어노테이션
@PropertySource("classpath:/config.properties")
class ImageDeleteScheduling {
	...
	public void scheduling() {
		...
		// 두 배열을 하나로 합침
		// imageArr라는 빈 배열을 boardArr과 길이 만큼의 크기로 만든다.
		File[] imageArr = new File[ memberArr.length + boardArr.length ];
		
		// 배열 내용 깊은 복사
		// System.arraycopy(복사할 배열, 몇번 인덱스부터 복사할지,새로운 배열, 새로운배열의몇번인덱스부터넣을지, 복사를어디까지할건지)
		...
		if(!serverImageList.isEmpty()) {// 서버에 이미지가 있을경우
...
*
*
*  @Scheduled(cron = "30 * * * * *") //  시계 초침이 30을 가리키는 경우 수행
*  @Scheduled(cron = "0 0 12 * * *") // 매 정오마다
*  @Scheduled(cron = "0,30 * * * * *") // 시계 초침이 0, 30을 가리키는 경우 수행
*  @Scheduled(cron = "0 0 24 * * *") // 매 자정마다
*  @Scheduled(cron = "0 0 * * * *") // 매 시간 정시(00분)마다
