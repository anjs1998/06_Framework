...
/*
 * Aspect Oriented Programming의 약자로, 분산되어 있는 관심사/관점을 모듈화하는 기법
 * 
 * - 주요 비즈니스 로직과 관련이 없는 부가적인 기능을 추가할떄 유용
 * 
 * ex) 코드 중간중간에 log찍을떄, transaction 처리하고 싶을때
 * 
 * 주요 어노테이션 
 * - @Aspect : Aspect를 정의하는데 사용되는 어노테이션으로, 클래스 상단에 작성된
 * - @Before(포인트컷) : 대상 메서드(포인트컷) 실행 전에 Advice를 실행
 * - @After(포인트컷) : 대상 메서드(포인트컷) 실행 후에 Advice를 실행
