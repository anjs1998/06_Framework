...
public class TestWebSocketHandler extends TextWebSocketHandler{
	...
	//WebSocketSession : client - server간 전이중(=양방향) 통신을 담당하는 객체
	...
	// 동기화를 진행하여 스레드가 순서대로 한 컬렉션에 접근할수 있도록 변경.
	
	//클라이언트와 서버가 연결이 완료되고, 통신할 준비가 되면 실행
	@Override
	public void afterConnectionEstablished(WebSocketSession session/*interceptor로 attributes.put("session", session)에서 가로챈 각체*/) throws Exception {
	...
	//클라이언트와 연결이 종료되면 실행
	@Override
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status/*종료할떄 상태코드*/) throws Exception {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		// message,getPayload() : 통신 시 탑재된 데이터 그자체
		log.info("전달받은 메시지 : {}", message.getPayload());
		
		
		// 전달받은 메시지를 
		// 현재 해당 웹소켓에 연결되어 있는 모든 클라이언트에에 보내기
