...
@Configuration // 서버 실행시 내부 작성된 메서드를 모두 수행
@EnableWebSocket // 웹소켓 활성화 설정
@RequiredArgsConstructor
public class WebSocketConfig implements WebSocketConfigurer{

	
	//Bean으로 등록된 SessionHandshakeInterceptor  DI주입
	private final HandshakeInterceptor handshakeInterceptor; 
	//Bean으로 등록된 TestWebSocketHandler DI주입
	...
	public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
		...
		//ws:// localhost/testSsock으로
		//클라이언트가 요청을 하면
		//estSocketHAndler가 처리하도록 등록
		.addInterceptors(handshakeInterceptor)
		// 클라이언트 연결 시 HttpSession을 가로채 핸들러에 전달하는 handshakeInterceptor 등록
