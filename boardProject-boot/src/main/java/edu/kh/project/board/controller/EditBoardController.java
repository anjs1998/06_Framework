...
import edu.kh.project.board.model.service.EditBoardServiceImpl;
...
public class EditBoardController {
	...
	@Autowired
	private EditBoardService service = new EditBoardServiceImpl();
	...
	public String boardInsert(@PathVariable("boardCode") int boardCode,
								...
								/*이미지*/	@RequestParam("images") List<MultipartFile> images,
								RedirectAttributes ra) throws Exception{
		
		/*List<MultipartFile> images
		 * - 5개 모두 업로드 했을때 -> List 안에 0~4번 인덱스에 파일 저장됨
		 * - 5개 모두 업로드 안헀을떄 -> List 안에 0~4번 인덱스에 파일 저장 X
		 * - 2번 인덱스만 업로드 -> 2번 인덱스만 파일 저장, 0/1/3/4번 인덱슨느 저장 X
		 * 
		 * [문제점]
		...
		// -> 성공 시 [상세조회]를 요청할수 있도록 삽입된 게시글의 번호를 반환받기
