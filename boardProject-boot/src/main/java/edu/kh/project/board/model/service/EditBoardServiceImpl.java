...
public class EditBoardServiceImpl implements EditBoardService {
	...
	public int boardInsert(Board inputBoard, List<MultipartFile> images) throws Exception{
		...
		if( uploadList.isEmpty() ) {
			return boardNo; // 컨트롤러로 현제 제목/상세내용 삽입된 게시글 번호 리턴
		}
		
		//선택한 파일이 존재할 경우
		// -> "BOARD_IMG" 테이블에 insert + 서버에 파일 저장
		result = mapper.insertUploadList(uploadList); // result == 삽입된 행의 개수 == uploadList.size()
		...
		if(result == uploadList.size()) {
			...
			//exception 강제발생시켜서 @Transactional선에서 rollback
	...
	public int boardUpdate(Board inputBoard, List<MultipartFile> images, String deleteOrderList) throws Exception{
		...
		// 2. 기존 0 -> 삭제된 이미지(deleteOrderList) 가 있는경우
		...
		for(int i = 0 ; i < images.size(); i++) {
			...
			if(!images.get(i).isEmpty()) {
				...
				// 4. 업로드 하려는 이미지 정보를 이용해서 수정 또는 삽입
		...
		for(BoardImg img : uploadList) {
			img.getUploadFile().transferTo(new File(folderPath));
		}
		return result;
	}
