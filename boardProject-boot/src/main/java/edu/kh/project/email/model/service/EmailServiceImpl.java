...
@Service
@Slf4j
@RequiredArgsConstructor // (lombok) final 필드에 자동으로 의존성 주입
public class EmailServiceImpl implements EmailService{
	
	private final EmailMapper mapper;
	private final JavaMailSender mailSender; // Emailconfig에서 Bean으로 등록된 JavamailSender를 이 변수에 저장
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			e.printStackTrace();
			return null; // 메일 발생 실패시 null 반환
		}
		
	}
	// HTML 템플릿에 데이터를 바인딩하여 최종 HTML 생성 메서드
	private String loadHtml(String authKey, String htmlName) {
		//Context : 타임리프를 제공하는 HTML템플릿에 데이터를
		//전달하기 위해 사용하는 클래스
		Context context = new Context();
		context.setVariable("authKey", authKey); // 템플릿에서 사용할 변수 authKey에 값 저장
	...
	@Transactional(rollbackFor=Exception.class) // 메서드 레벨에서도 이용 가능하다.(해당 메서드에서만 트랜잭션 커밋/롤백)
	...
	// 주로 데이터베이스 기본키, 고유 한 식별자를 생성해야 할때 사용
